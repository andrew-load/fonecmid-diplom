...
Процедура ВыполнитьАвтозаполнение(ДатаДокумета, Договор, Реализация) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(ВыполненныеКлиентуРаботыОбороты.КоличествоЧасовОборот) КАК КоличествоЧасовОборот,
		|	СУММА(ВыполненныеКлиентуРаботыОбороты.СуммаКОплатеОборот) КАК СуммаКОплатеОборот
		|ИЗ
		|	РегистрНакопления.ВыполненныеКлиентуРаботы.Обороты(&ДатаНачала, &ДатаОкончания,, ДоговорКонтрагента = &Ссылка) КАК
