...
Процедура РассчитатьУдержание()
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	МАКСИМУМ(ЕСТЬNULL(ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.РезультатБаза, 0)) КАК РезультатБаза,
				   |	МАКСИМУМ(ЕСТЬNULL(ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.ОтработаноДней, 0)) КАК ОтработаноДнейФакт,
				   |	МАКСИМУМ(ЕСТЬNULL(ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.ОтработаноДнейБаза, 0)) КАК ОтработаноДнейБаза,
				   |	СУММА(ЕСТЬNULL(ВКМ_ДополнительныеНачисленияБазаВКМ_ОсновныеНачисления.Результат, 0)) КАК ДопНачисления
				   |ИЗ
				   ...
				   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ДополнительныеНачисления.БазаВКМ_ОсновныеНачисления(&Измерения, &Измерения, , ) КАК ВКМ_ДополнительныеНачисленияБазаВКМ_ОсновныеНачисления
				   |		ПО (ВКМ_ОсновныеНачисления.ПериодРегистрации = ВКМ_ДополнительныеНачисленияБазаВКМ_ОсновныеНачисления.ПериодРегистрации)
				   |			И (ВКМ_ОсновныеНачисления.Сотрудник = ВКМ_ДополнительныеНачисленияБазаВКМ_ОсновныеНачисления.Сотрудник)
				   |ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ДвижениеВзаиморасчет = Движения.ВКМ_ВзаиморасчетыССотрудниками[ВыборкаДетальныеЗаписи.НомерСтроки - 1];

		РезультатДвижения = (((ВыборкаДетальныеЗаписи.РезультатБаза + ВыборкаДетальныеЗаписи.ДопНачисления)
			/ ВыборкаДетальныеЗаписи.ОтработаноДнейБаза) * ВыборкаДетальныеЗаписи.ОтработаноДнейФакт) * (НДФЛ / 100);
		Движение.Результат = РезультатДвижения;
		
		// регистр ВКМ_ВзаиморасчетыССотрудниками
		ДвижениеВзаиморасчет.Сумма = РезультатДвижения;
	КонецЦикла;
